...
for k=1:length(ff)
    for i=1:n_f_generator{k}
        ...
        % con = [con,P_gen(f_gen{k}(i),min(T,ceil(2.5*SAET{k}(i))))==0];% 停机约束
    end
    % con = [con,sum(sum(P_gen(f_gen{k},1:min(T,ceil(2.5*SAET{k}(i)))),2)./f_gen_ratio{k})<=ALP_0{k}(i)];% ALP约束
end
...
% for k=1:6
%     ALP_r{k} = ALP_0{k}' - sum(sum(z_gen(f_gen{k},:), 2)./f_gen_ratio{k}); % kg
% end